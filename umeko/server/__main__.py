...
def _bootstrap_admin(store, logger: logging.Logger) -> None:
    """没有任何管理员时，用环境变量创建首个 admin；否则告警提示。"""
    if store.has_admin():
        return
    username = os.getenv("UMEKO_ADMIN_USERNAME", "admin")
    password = os.getenv("UMEKO_ADMIN_PASSWORD", "")
    if password:
        store.create_user(username, password, role="admin")
        logger.info("已创建首个管理员账号：%s", username)
    else:
        logger.warning(
            "当前没有任何管理员账号，管理端口无法登录；"
            "请设置 UMEKO_ADMIN_PASSWORD 后重启以创建首个管理员"
        )
